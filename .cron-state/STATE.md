...
## STATUS: DEVELOP — `feature/v0.13.0-lens`, Slice 6 of 9 DONE (Auto-tag on import)

**v0.13.0 Lens slices progress (5 / 9):**
...
- ✅ **Slice 6: Auto-tag on Import** — vertical slice in 3 commits. (a) `1e86442` — new `ai::auto_tag` module (346 LOC) with `AutoTagOpts { max_tags=5, max_context_chars=6_000 }`, `auto_tag(provider, text, opts)` and `auto_tag_from_path(provider, pdf, opts)` orchestrators (extract text via `pdf::extract`, build chat prompt with temp=0.1 + max_tokens=64 for deterministic short replies), `parse_tag_reply` (strips bullets/quotes/numeric prefixes, lower-cases, dedupes preserving order, drops empties + >32-char outliers, caps at clamped max). 8 unit tests cover happy path, prefix stripping, dedupe, max-tag clamp, empty reply, long-string drop, prompt structure assertions. (b) `055236b` — new `pdf::library::auto_tagger` orchestrator (310 LOC) bridging the AI module to the registry: `AutoTagRunResult { doc_id, tags_assigned, tag_ids, error }`, `run_one(db, provider, doc_id, opts)` and `run_many(db, provider, doc_ids, opts)`. **Additive contract**: `final = existing ∪ suggested`, never removes a user-set tag. Per-doc failures collapse into `error: Some(...)` so bulk runs never abort (same pattern as OCR queue). 6 tests cover assignment, additive preservation, case-insensitive dedupe, missing-doc error, extract failure, run_many continuing past per-doc failure. 2 new Tauri commands `slab_library_auto_tag_one(doc_id, opts?)` and `slab_library_auto_tag_many(doc_ids, opts?)` (async, follow `slab_beacon_*` provider-construction pattern via `ai::config::make_provider`). TS bindings in `src/lib/library.ts` — `AutoTagOpts`, `AutoTagRunResult`, `autoTagRunOne`, `autoTagRunMany`. **332 → 346 lib tests** (+14). (c) `c760916` — LibraryPanel.svelte UI: toolbar "🏷️ Auto-tag N" button, per-card "🏷️ Auto-tag" action (works on all docs, not just OCR candidates), context-menu entry; `applyAutoTagResult` optimistic patch + tag-rail refresh on success; bulk action has confirm() guard + summary banner ("✓ Auto-tag: K tagged, F failed"); friendly error mapping when 0/N succeed pointing at Beacon provider config. All 4 quality gates clean: fmt + clippy `-D warnings` + 346 lib tests + svelte-check 0 errors.
- ⏳ **Slice 4, 7+: equation → LaTeX (pix2tex sidecar), tesseract DPI tuning, mixed-OCR overlay, CLI surface, release prep** — see `.cron-state/proposals/v0.13.0-lens.md`

**Active branch:** `feature/v0.13.0-lens` (3 new commits this tick: `1e86442`, `055236b`, `c760916` — Slice 6 end-to-end)
**Last shipped tick:** Slice 6 Auto-tag on Import — vertical slice (ai module + 8 tests → orchestrator + 6 tests → 2 Tauri cmds → TS bindings → LibraryPanel toolbar/card/menu UI)
