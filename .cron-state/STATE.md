...
## STATUS: 🚑 v2.1.0 CI hotfix shipped — retagged, new runs in_progress, MODE B next tick

**v2.1.0 STATUS**: 🚑 **CI HOTFIX SHIPPED** — initial CI runs (`26271002054`+`26270991776`) both failed: Dockerfile used wrong rust tag (`rust:1.83-bookworm-slim` doesn't exist, fixed to `slim-bookworm`) and Cargo autobin from `src/bin/server.rs` confused tauri-bundler into copying a nonexistent `release/server` binary on desktop builds. Fix commit `73ebf45` moved source to `src/server_main.rs` + Dockerfile tag fix. Tag `v2.1.0` deleted + re-pushed to new SHA. New CI runs: `26272340751` (Docker), `26272331779` (build). MODE B finalize next tick (poll + artifact upload + mark release non-draft). GH release still in draft state.
